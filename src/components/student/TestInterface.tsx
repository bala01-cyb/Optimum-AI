...
const TestInterface: React.FC = () => {
  ...
  const fetchTestData = async () => {
    ...
    try {
      ...
      // 6. Initialize or resume adaptive testing state
      ...
      // 7. Fetch the first/next question using adaptive system
      const nextQuestion = await adaptiveTestService.fetchNextQuestion(testId, userData.uid, currentAdaptiveState);
      if (nextQuestion) {
        setCurrentQuestion(nextQuestion);
        setQuestionStartTime(Date.now());
      } else {
        console.warn('No questions available for adaptive testing');
        setTestEnded(true);
        setLoading(false);
        return;
      }

      // 8. Handle timing - ALWAYS start with full time initially
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-28 sm:pb-16">
        ...
        {!deviceIsMobile ? (
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <Shield className={`h-5 w-5 ${
                  proctorState.isFullscreen ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
                }`} />
                <div>
          ...
          <div className={`border rounded-lg p-4 mb-6 ${
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <Shield className={`h-5 w-5 ${
                  proctorState.violationCount > 0 ? 'text-red-600' : 'text-orange-600'
                }`} />
                <div>
                  <p className={`text-sm font-medium ${
                    proctorState.violationCount > 0 ? 'text-red-900' : 'text-orange-900'
                  }`}>
                    Mobile Anti-Cheat Active
                  </p>
                  <p className={`text-xs ${
                    proctorState.violationCount > 0 ? 'text-red-700' : 'text-orange-700'
                  }`}>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
            ...
            <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-lg p-4 mb-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-6 text-sm">
                  ...
                  <div className="text-center">
                    <div className="font-semibold text-purple-600">{adaptiveState.askedQuestionIds.length}</div>
          ...
          <div className="flex items-center justify-between text-sm text-gray-600">
            {isAdaptiveMode ? (
              <span>Adaptive Question #{adaptiveState?.askedQuestionIds.length + 1 || 1}</span>
            ) : (
              <span>Question {currentQuestionIndex + 1} of {questions.length}</span>
            )}
            <span>Answered: {getAnsweredCount()}/{isAdaptiveMode ? '∞' : questions.length}</span>
          </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-3">
              ...
              <span className="text-sm font-medium text-gray-600">
                {isAdaptiveMode 
                  ? `Adaptive Question #${adaptiveState?.askedQuestionIds.length + 1 || 1}`
          ...
          <div className="mb-6">
            {isAdaptiveMode && currentQuestion ? (
              ...
              !isAdaptiveMode && questions.length > 0 && renderQuestionContent(questions[currentQuestionIndex])
            )}
          </div>

          <div className="space-y-3">
            {(isAdaptiveMode ? currentQuestion : questions[currentQuestionIndex])?.options.map((option, index) => {
              const questionToUse = isAdaptiveMode ? currentQuestion : questions[currentQuestionIndex];
              return (
                <button
                  key={index}
                  ...
                  className={`w-full text-left p-4 rounded-lg border transition-all duration-200 ${
                    answers[questionToUse?.id] === index
        ...
        {/* Navigation - Different for Adaptive Mode */}
        {!isAdaptiveMode ? (
          <div className="flex items-center justify-between">
            <button
              ...
              {currentQuestionIndex === questions.length - 1 ? (
                <div className="relative">
                  <button
                    ...
                    className={`px-6 py-2 text-white rounded-md disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 flex items-center space-x-2 ${
                  ...
                  >
                    <CheckCircle className={`h-4 w-4 ${
                      showIncompleteWarning ? 'animate-bounce' : ''
                    }`} />
                    <span>
                      {submitting 
                        ? 'Submitting...' 
          ...
          // Adaptive Mode - Show submit button and progress info
          <div className="flex items-center justify-center">
            <button
              onClick={handleSubmitTest}
              disabled={submitting || testEnded || timeLeft === 0}
              className="px-8 py-3 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-lg font-medium transition-all duration-300 flex items-center space-x-3 hover:from-purple-700 hover:to-blue-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
            >
              <CheckCircle className="h-5 w-5" />
              <span>
                {submitting ? 'Submitting Test...' : 'Submit Adaptive Test'}
              </span>
            </button>
          </div>
        )}

        {/* Question Navigation - Only for Non-Adaptive Mode */}
        {!isAdaptiveMode && (
        ...
        {isAdaptiveMode && adaptiveState && adaptiveState.difficultyFlow.length > 0 && (
          <div className="mt-6 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
