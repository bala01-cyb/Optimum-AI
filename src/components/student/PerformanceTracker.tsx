    ...
    return (
        <div className="card-modern glass p-4 mb-4">
            ...
            <div className="flex items-center mb-3">
                <div className="flex items-center justify-center w-10 h-10 rounded-lg bg-gradient-to-br from-purple-500 to-blue-500 mr-3">
                    <BarChart3 className="h-5 w-5 text-white" />
                </div>
                <div>
                    <h3 className="text-base font-semibold text-gray-900 dark:text-white">Performance Tracker</h3>
                    <p className="text-xs text-gray-600 dark:text-gray-400">
                        Your progress over the last {history.length + 1} test{history.length !== 0 ? 's' : ''}
                    </p>
                </div>
            </div>

            {/* Insight Banner */}
            <div className={`rounded-lg p-3 border mb-3 ${getTrendColor()}`}>
                <div className="flex items-start gap-2">
                    <div className="flex-shrink-0 mt-0.5">{getTrendIcon()}</div>
                    <div className="flex-1">
                        <p className="text-sm font-medium">{insights.message}</p>
                        <p className="text-xs mt-1 opacity-90">{insights.recommendation}</p>
                    </div>
                </div>
            </div>
            ...
            {insights.badge && (
                <div className="mb-3 flex justify-center">
                    <div className="inline-flex items-center px-3 py-1.5 rounded-full bg-gradient-to-r from-yellow-400 to-orange-400 text-white text-sm font-semibold shadow">
            ...
            {trend.trend !== 'insufficient-data' && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
                    <div className="text-center p-2.5 rounded-lg bg-gray-50 dark:bg-gray-800/30 border border-gray-200 dark:border-gray-700/50">
                        <div className="flex items-center justify-center w-8 h-8 bg-blue-100 dark:bg-blue-900/30 rounded-lg mx-auto mb-1.5">
                            <Target className="h-4 w-4 text-blue-600 dark:text-blue-400" />
                        </div>
                        <p className="text-xl font-bold text-gray-900 dark:text-white">{trend.averageScore}%</p>
                        <p className="text-xs text-gray-600 dark:text-gray-400">Average</p>
                    </div>

                    <div className="text-center p-2.5 rounded-lg bg-gray-50 dark:bg-gray-800/30 border border-gray-200 dark:border-gray-700/50">
                        <div className="flex items-center justify-center w-8 h-8 bg-green-100 dark:bg-green-900/30 rounded-lg mx-auto mb-1.5">
                            <Award className="h-4 w-4 text-green-600 dark:text-green-400" />
                        </div>
                        <p className="text-xl font-bold text-gray-900 dark:text-white">{trend.bestScore}%</p>
                        <p className="text-xs text-gray-600 dark:text-gray-400">Best</p>
                    </div>

                    <div className="text-center p-2.5 rounded-lg bg-gray-50 dark:bg-gray-800/30 border border-gray-200 dark:border-gray-700/50">
                        <div className="flex items-center justify-center w-8 h-8 bg-purple-100 dark:bg-purple-900/30 rounded-lg mx-auto mb-1.5">
                            <Zap className="h-4 w-4 text-purple-600 dark:text-purple-400" />
                        </div>
                        <p className="text-xl font-bold text-gray-900 dark:text-white">{trend.consistency}%</p>
                        <p className="text-xs text-gray-600 dark:text-gray-400">Consistency</p>
                    </div>

                    <div className="text-center p-2.5 rounded-lg bg-gray-50 dark:bg-gray-800/30 border border-gray-200 dark:border-gray-700/50">
                        <div className="flex items-center justify-center w-8 h-8 bg-orange-100 dark:bg-orange-900/30 rounded-lg mx-auto mb-1.5">
            ...
            {history.length > 0 && (
                <div>
                    <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Recent Test History</h4>
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-gray-200 dark:border-gray-700">
                                    <th className="text-left py-1.5 px-2 text-xs text-gray-600 dark:text-gray-400 font-medium">Test</th>
                                    <th className="text-center py-1.5 px-2 text-xs text-gray-600 dark:text-gray-400 font-medium">Score</th>
                                    <th className="text-center py-1.5 px-2 text-xs text-gray-600 dark:text-gray-400 font-medium">Date</th>
                                </tr>
                            </thead>
                            <tbody>
                                    ...
                                    <tr key={item.testId} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                        <td className="py-1.5 px-2 text-gray-900 dark:text-white">
                                            <div className="flex items-center">
                                                <span className="w-5 h-5 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-xs font-medium mr-2">
                                                    {index + 1}
                                                </span>
                                        ...
                                        <td className="py-1.5 px-2 text-center">
                                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold ${item.percentage >= 80
                                        ...
                                        <td className="py-1.5 px-2 text-center text-xs text-gray-600 dark:text-gray-400">
